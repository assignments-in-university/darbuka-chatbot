...
<template>
  ...
  <div class="w-full h-[calc(100vh-96px)] overflow-y-auto scrollbar-thumb-emerald p-4" v-else>
    ...
    >
      <div class="flex items-center gap-x-4 mb-4 font-primary">
        <motion.span
      ...
      <motion.h2
        class="font-primary lg:max-w-1/2 text-4xl font-medium mb-4 text-emerald"
        :initial="{ opacity: 0, y: 10 }"
        :animate="{ opacity: 1, y: 0 }"
        :transition="{ delay: 0.1 }"
      >
    ...
    <div class="flex gap-x-4 px-10 max-w-3xl mx-auto">
      <motion.div class="w-24" :initial="{ height: 0 }" :animate="{ height: 'auto' }" :transition="{ delay: 0.2, duration: 1 }">
        <div class="w-1 bg-neutral-700 rounded-full h-full mx-auto"></div>
      </motion.div>
      <div class="w-full space-y-24 py-10">
        <motion.div
        ...
        >
          ...
          <h3 class="text-3xl font-semibold text-verdant mb-4">{{ lesson.title }}</h3>
          <p>{{ lesson.description }}</p>

          <!-- Is Completed Indicator -->
          <div
            class="absolute size-16 border-2 border-emerald bg-emerald rounded-full top-1/2 -translate-y-1/2 left-0 -translate-x-[calc(100%+26px)] shadow-[0px_0px_20px_10px] shadow-verdant/10 flex items-center justify-center duration-150 transition-colors"
            :class="{
    ...
    <button
      class="w-full max-w-2xl font-primary bg-emerald text-black px-4 py-2 rounded-md mt-12 mx-auto font-medium cursor-pointer flex gap-x-2 justify-center items-center duration-150 transition-all"
